...
usage() {
    echo "Usage: run-cageforge-command-linux-vm.sh --image IMAGE --test-bundle BUNDLE" >&2
    exit 64
}
...
set -euo pipefail
bundle_root=$(mktemp -d /home/ubuntu/boss-command-bundle.XXXXXX)
...
bash "$bundle_root/ci/cageforge-command-suite/run.sh"
